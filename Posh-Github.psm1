...
function Remove-GitHubIssueLabel {
    ...
    $uri = ("https://api.github.com/repos/$Owner/$Repository/issues/$Issue/labels/$Label")
    Write-Host "Removing label $Label from issue $Issue"
    Write-Host $uri
    ...
    $delResult = Invoke-RestMethod @params 
    $global:GITHUB_API_OUTPUT = $delResult.items 

    $global:GITHUB_API_OUTPUT |
    % { Write-Host "Removed label for $($_.number): $($_.title)" }
...
function GetGitHubRepos($SearchType, $Name, $Type, $Sort, $Direction) {
    switch ($SearchType) {
        'org' {
            $uri = "https://api.github.com/orgs/$Name/repos" 
        }
        'user' {
...
function Get-GitHubRepositories {
    ...
    try {
        ...
        if ($Direction -eq $null) {
            $Direction = if ($Sort -eq 'full_name') {
                'asc' 
            } else {
                'desc' 
            }
        ...
        $global:GITHUB_API_OUTPUT |
        % {
            $size = if ($_.size -lt 1024) {
                "$($_.size) KB" 
            }
            else {
                "$([Math]::Round($_.size, 2)) MB"
            }
...
Get-GitHubStatus, Set-GitHubStatus, Search-GitHubRepoIssues, Remove-GitHubIssueLabel
